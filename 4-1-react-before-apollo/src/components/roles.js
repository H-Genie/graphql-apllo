    ...
    function MainContents() {
        ...
        return (
            <div className="roleWrapper">
                <h2>{data.role && data.role.id}</h2>
                <div className="requirement"><span>{data.role && data.role.requirement}</span> required</div>
                <h3>Members</h3>
                <ul>
                    {data.role && data.role.members.map((member) => {
                        return (<li>{member.last_name}</li>)
                    })}
                </ul>
                <h3>Equipments</h3>
                <ul>
                    {data.role && data.role.equipments.map((equipment) => {
                        return (<li>{equipment.id}</li>)
                    })}
                </ul>
                <h3>Softwares</h3>
                {data.role && data.role.softwares.map((software) => {
